...
if __name__ == "__main__":
    ...
    for filename in filenames:
        rf = scipy.io.loadmat(filename)['rf'].T
        for line_n in range(rf.shape[0]):
            line = rf[line_n, START:END]
            line = preprocess_line(line, decimate=args.decimate)
    ...
    with h5py.File(os.path.join(args.dataset_dir, args.output_file), 'w') as output_f:
        ...
        for filename in filenames:
            ...
            print(alpha)
            print(scipy.io.loadmat(filename)['rf'].shape)
            rf = scipy.io.loadmat(filename)['rf'].T
            for line_n in range(rf.shape[0]):
                line = preprocess_line(rf[line_n, START:END], decimate=args.decimate)
                t = 0
                chunks = []
